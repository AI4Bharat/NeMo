...
def viterbi_decoding(log_probs_batch, y_batch, T_batch, U_batch, viterbi_device):
    ...
    padding_for_log_probs = V_NEGATIVE_NUM * torch.ones(
        (B, T_max, 1), device=viterbi_device
    )
    ...
    v_prev[:, :2] = torch.gather(
        input=log_probs_padded[:, 0, :], dim=1, index=y_batch[:, :2]
    )
    ...
    backpointers_rel = -99 * torch.ones(
        (B, T_max, U_max), dtype=torch.int8, device=viterbi_device
    )
    ...
    for t in range(1, T_max):
        ...
        U_can_be_final = torch.logical_or(
            torch.arange(0, U_max, device=viterbi_device).unsqueeze(0)
            == (U_batch.unsqueeze(1) - 0),
            torch.arange(0, U_max, device=viterbi_device).unsqueeze(0)
            == (U_batch.unsqueeze(1) - 1),
        )
    ...
    for b in range(B):
        ...
        if (
            U_b == 1
        ):  # i.e. we put only a blank token in the reference text because the reference text is empty
            current_u = (
                0  # set initial u to 0 and let the rest of the code block run as usual
            )
        else:
