...
def get_char_tokens(text, model):
    ...
    for character in text:
        if character in model.decoder.vocabulary:
            ...
            tokens.append(
                len(model.decoder.vocabulary)
            )  # return unk token (same as blank token)
    ...
    for token in word_tokens:
        ...
        for token_char in token:
            if token_char == word[word_char_pointer]:
                ...
                if token_char.upper() == word[
                    word_char_pointer
                ] or is_sub_or_superscript_pair(token_char, word[word_char_pointer]):
...
def get_utt_obj(
    ...
    if (
        not separator
    ):  # if separator is not defined - treat the whole text as one segment
        segments = [text]
    else:
    ...
    if hasattr(model, "tokenizer"):
        if hasattr(model, "blank_id"):
            ...
            BLANK_ID = len(model.tokenizer.tokenizers_dict[language_id].vocab)  # TODO: check
        ...
        all_tokens = model.tokenizer.tokenizers_dict[language_id].text_to_ids(text)
        ...
        for segment in segments:
            ...
            segment_tokens = model.tokenizer.tokenizers_dict[language_id].text_to_tokens(segment)
            utt.segments_and_tokens.append(
                Segment(
                    text=segment,
            ...
            words = segment.split(
                " "
            )  # we define words to be space-separated sub-strings
            for word_i, word in enumerate(words):
                word_tokens = model.tokenizer.tokenizers_dict[language_id].text_to_tokens(word)
                word_token_ids = model.tokenizer.tokenizers_dict[language_id].text_to_ids(word)
    ...
    ):  # i.e. tokenization is simply character-based
        ...
        for i_segment, segment in enumerate(segments):
            ...
            words = segment.split(
                " "
            )  # we define words to be space-separated substrings
            for i_word, word in enumerate(words):
                ...
                for token_i, (token, token_id) in enumerate(
                    zip(word_tokens, word_token_ids)
                ):
                    # add the text tokens and the blanks in between them
                    # to our token-based variables
                    ...
                    if (
                        token_i < len(word_tokens) - 1
                    ):  # only add blank tokens that are in the middle of words
...
def add_t_start_end_to_utt_obj(utt_obj, alignment_utt, output_timestep_duration):
    ...
    for segment_or_token in utt_obj.segments_and_tokens:
        if type(segment_or_token) is Segment:
            ...
            segment.t_start = (
                num_to_first_alignment_appearance[segment.s_start]
                * output_timestep_duration
            )
            segment.t_end = (
                num_to_last_alignment_appearance[segment.s_end] + 1
            ) * output_timestep_duration
            ...
            for word_or_token in segment.words_and_tokens:
                if type(word_or_token) is Word:
                    word = word_or_token
                    word.t_start = (
                        num_to_first_alignment_appearance[word.s_start]
                        * output_timestep_duration
                    )
                    word.t_end = (
                        num_to_last_alignment_appearance[word.s_end] + 1
                    ) * output_timestep_duration

                    for token in word.tokens:
                        if token.s_start in num_to_first_alignment_appearance:
                            token.t_start = (
                                num_to_first_alignment_appearance[token.s_start]
                                * output_timestep_duration
                            )
                        else:
                            token.t_start = -1
...
def get_batch_variables(
    ...
    for i_line, line in enumerate(manifest_lines_batch):
        ...
        if align_using_pred_text:
            ...
            if "text" in line:
                utt_obj.text = line[
                    "text"
                ]  # keep the text as we will save it in the output manifest
        else:
    ...
    if hasattr(model, "tokenizer"):
        V = len(model.tokenizer.tokenizers_dict[language_id].vocab) + 1
    else:
